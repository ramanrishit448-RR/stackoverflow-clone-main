...
const index = () => {
  ...
  const { Login, verifyDeviceLogin, loading } = useAuth();
  ...
  const handlesubmit = async (e: any) => {
    ...
    if (res && res.status === "pending_otp") {
      setSessionId(res.sessionId);
      setOtpMode(true);
      console.log("DEBUG_OTP_CODE:" + res.debugOTP);
  ...
  const handleVerifyOtp = async (e: any) => {
    ...
    const success = await verifyDeviceLogin(sessionId, otpCode);
    if (success) {
      router.push("/");
    }
  };
          ...
          <form onSubmit={handleVerifyOtp}>
            <Card className="border border-orange-200 shadow-lg">
              <CardHeader className="space-y-1 text-center">
                <CardTitle className="text-xl lg:text-2xl text-orange-600 flex items-center justify-center gap-2">
                  🔒 Unrecognized Device
                </CardTitle>
                <CardDescription>
                  Enter the 6-digit verification code sent to your registered email to approve this login.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={otpCode}
                    className="text-center font-mono font-bold tracking-[0.4em] text-xl py-6"
                  />
                </div>
                <Button
                  type="submit"
